...
function createAPIRequest(parameters, callback) {
  ...
  // if we set gzip on the request, stringify and gzip the body, and set the request headers.
  if (parameters.gzip) {
    if (parameters.gzipedJSON) {
      options.body = parameters.gzipedJSON;
    }
    else {
      options.body = zlib.gzipSync(JSON.stringify(options.json));
    }
